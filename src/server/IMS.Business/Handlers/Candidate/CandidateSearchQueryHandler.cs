...
public class CandidateSearchQueryHandler(IUnitOfWork unitOfWork, IMapper mapper) :
    BaseHandler(unitOfWork, mapper),
    IRequestHandler<CandidateSearchQuery, PaginatedResult<CandidateViewModel>>
{
    public async Task<PaginatedResult<CandidateViewModel>> Handle(
        CandidateSearchQuery request,
